...
#
# scale_rewards setting added for drgrpo 2025-04-27
# L.449, L.899-901 
# make trainable 2025-05-15
# L.580, 1242
...
#####-----------------------------make trainable
        ...
        force_enable_grad = getattr(args, 'force_enable_grad', False)
        ...
        if force_enable_grad:
            ...
            if self.accelerator.is_main_process:
                ...
                print(f"Force-enabled gradients: {trainable}/{total} parameters ({trainable/total:.1%})")
...
#####-----------------------------
    ...
    ###-------------------------------make trainable
        ...
        if self.state.global_step == 0:
            # 初回ステップで勾配状態を確認
            grad_status = [p.requires_grad for p in self.model.parameters()]
            print(f"Gradient status at step 0: {sum(grad_status)}/{len(grad_status)}")
